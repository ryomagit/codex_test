...
- [~] 認証 UI: ログイン/登録は動くが、メイン画面の常設フォームとして表示されていて目立ちすぎる。Next: `Frontend UI improvement checklist` の P0 認証分離から着手する。
- [~] 画面構成: `App.vue` にログイン、検索、詳細、レビュー、ランキング、ユーザー機能が全部縦積みされている。Next: 主要画面または明確なセクションへ再構成する。
...
- [ ] P0 認証 UI を分離する: ログイン/登録をメイン画面の常設フォームから外し、未ログイン時だけ自然に出る認証ビューまたは認証パネルにする。Next: 未ログイン時とログイン後の header / account 表示を設計する。
- [ ] P0 主要画面を再構成する: `Discover`、`Library`、`Book Detail`、`People` などの画面または明確なセクションへ分ける。Next: どの状態をどの画面に置くかを `App.vue` 上で整理する。
- [ ] P0 初期表示を改善する: `App.vue` に全機能が縦積みされ、初期表示がフォームだらけに見える状態を解消する。Next: 初期表示は検索/発見導線を中心にし、補助機能は必要な時に見せる。
