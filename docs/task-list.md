...
- [x] 状態表示: 読み込み中、成功、失敗に加え、主要一覧の空状態、未ログイン時の誘導、文脈別のエラー/成功通知を整理した。Next: 通知文言を利用状況に応じて磨く。
...
- [x] P1 空状態を設計する: 検索結果なし、お気に入りなし、レビューなし、ランキングなし、ユーザーなしを copy と action で表示する。Next: 空状態の文言を利用状況に応じて磨く。
- [x] P1 未ログイン状態を設計する: お気に入り、レビュー、フォローなどログイン必須操作から認証パネルへ誘導する。Next: P1 エラー/成功通知の文脈別表示を整理する。
- [x] P1 エラー/成功通知を整理する: API エラーや保存成功を、認証、検索、詳細、レビュー、ライブラリ、ランキング、ユーザーの関連する文脈で表示する。Next: 通知文言を利用状況に応じて磨く。
- [x] P1 モバイル表示を改善する: nav、フォーム、カード、詳細、ランキング、ユーザー一覧が狭い幅でも詰まらないよう調整した。Next: P2 で redesign 後の表示確認手順を追加する。
